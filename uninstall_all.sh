...
# incluindo containers Docker, volumes, imagens, pacotes instalados e todos os arquivos
# e diretórios criados (por exemplo, o diretório do site).
...
echo "Removendo volumes Docker (ex: portainer_data)..."
...
echo "Removendo imagens Docker utilizadas (portainer/portainer-ce:latest e nginx:latest)..."
...
# Remover o diretório do site do usuário (em /home/<usuário>) se existir
if [ -n "$SUDO_USER" ]; then
...
# Remover o diretório do site em /root, se existir
ROOT_SITE_DIR="/root/meu-site"
if [ -d "$ROOT_SITE_DIR" ]; then
  echo "Removendo o diretório do site: $ROOT_SITE_DIR"
  rm -rf "$ROOT_SITE_DIR"
else
  echo "Diretório do site ($ROOT_SITE_DIR) não encontrado."
fi
